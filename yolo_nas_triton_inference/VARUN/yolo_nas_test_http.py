...
def preprocess_image(image_path: str, target_size: tuple = (640, 640)) -> Tuple[np.ndarray, tuple, np.ndarray, tuple, tuple]:
    ...
    return image,input_image, original_shape, resized_image, ratio, pad
...
def postprocess_output(num_detections: np.ndarray, boxes: np.ndarray, scores: np.ndarray, 
    ...
    # Directly iterate over the detections
    for i in range(num_dets):
        # Get scalar values from the arrays
        score = float(scores[0][i])  # Convert to Python float
        box = boxes[0][i]  # Get the box coordinates for this detection
        class_id = int(class_ids[0][i])  # Convert to Python int
        detections.append((box, score, class_id))
    
    return detections

def visualize_detections_on_original(original_image: np.ndarray, detections: list, ratio: tuple, pad: tuple) -> np.ndarray:
    """
    Visualize detections on the original image by reversing the letterbox resizing.
    
    Args:
        original_image: Original image before resizing
        detections: List of detections (box, score, class_id) from resized image
        ratio: Resize ratio used during preprocessing (width_ratio, height_ratio)
        pad: Padding added during resizing (pad_w, pad_h)
        
    Returns:
        Annotated original image with bounding boxes and labels
    """
    image = original_image.copy()
    h, w = image.shape[:2]
    
    for box, score, class_id in detections:
        # Revert letterbox scaling and padding
        x1 = int((box[0] - pad[0]) / ratio[0])
        y1 = int((box[1] - pad[1]) / ratio[1])
        x2 = int((box[2] - pad[0]) / ratio[0])
        y2 = int((box[3] - pad[1]) / ratio[1])
...
def main():
    ...
    output_dir = os.path.join(current_path, "Triton_HTTP_YOLONas_predictions_v10")
    ...
    for image_file in tqdm(image_files, desc="Inferencing", unit="image"):
        ...
        # Preprocess image
        try:
            original_image,input_image, original_shape, resized_image, ratio, pad = preprocess_image(image_path)
        ...
        result_image = visualize_detections_on_original(original_image, detections, ratio, pad)
