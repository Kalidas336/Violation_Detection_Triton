...
def infer_image(triton_client, image_path, user_data):
    """
    Submit a single image for asynchronous inference with CUDA shared memory
    ...
    Returns:
        Tuple of request details and shared memory handles if successful, None otherwise
    """
    try:
        # Preprocess image
        ...
        # Unique shared memory region names
        input_shm_name = f'input_{os.path.basename(image_path)}'
        output_shapes = [(1000, 4), (1000,), (1000,), (1,)]
        output_names = ["det_boxes", "det_scores", "det_classes", "num_dets"]
        output_types = [np.float32, np.float32, np.int32, np.int32]
        
        # Create and register input shared memory
        input_byte_size = input_data.size * input_data.itemsize
        shm_input_handle = cudashm.create_shared_memory_region(
            input_shm_name, input_byte_size, 0
        )
        cudashm.set_shared_memory_region(shm_input_handle, [input_data])
        triton_client.register_cuda_shared_memory(
            input_shm_name, cudashm.get_raw_handle(shm_input_handle), 0, input_byte_size
        )
        
        # Create and register output shared memories
        output_shm_handles = []
        for name, shape, dtype in zip(output_names, output_shapes, output_types):
            output_shm_name = f'{name}_{os.path.basename(image_path)}'
            output_byte_size = np.prod(shape) * np.dtype(dtype).itemsize
            shm_handle = cudashm.create_shared_memory_region(
                output_shm_name, output_byte_size, 0
            )
            triton_client.register_cuda_shared_memory(
                output_shm_name, cudashm.get_raw_handle(shm_handle), 0, output_byte_size
            )
            output_shm_handles.append((output_shm_name, shm_handle, output_byte_size, shape, dtype))
        
        # Prepare inference inputs and outputs
        inputs = []
        ...
        outputs = []
        for name, _, output_byte_size, _, _ in output_shm_handles:
            outputs.append(grpcclient.InferRequestedOutput(name.split('_')[0]+"_"+name.split('_')[1]))
            outputs[-1].set_shared_memory(name, output_byte_size)
        ...
        return request_id, request_time, original_shape, original_image, ratio, pad, input_shm_name, shm_input_handle, output_shm_handles
...
def main():
    ...
    input_dir = r"/home/user/VARUN/Benchmarking_YOLONAS/yolo_nas_tensorrt_inference/Test_dataset"
    output_dir = os.path.join(current_path, "YOLO_NAS_gRPC_Async_CUDA_TensorRT")
    ...
    try:
        ...
        with grpcclient.InferenceServerClient(url=SERVER_URL) as triton_client:
            ...
            for image_path in tqdm(image_paths, desc="Submitting requests", unit="image"):
                ...
                if result:
                    (request_id, request_time, original_shape, original_image, 
                     ratio, pad, input_shm_name, shm_input_handle, output_shm_handles) = result
                    ...
                    request_ids[request_id] = {
                        ...
                        "output_shm_handles": output_shm_handles
                    }
            ...
            for _ in range(len(request_ids)):
                ...
                if isinstance(data[0], InferenceServerException):
                    ...
                    if this_id in request_ids:
                        ...
                        input_shm_name = request_ids[this_id]["input_shm_name"]
                        shm_input_handle = request_ids[this_id]["shm_input_handle"]
                        output_shm_handles = request_ids[this_id]["output_shm_handles"]
                        
                        # Retrieve results from shared memory
                        output_data = {}
                        for (output_shm_name, shm_handle, _, shape, dtype) in output_shm_handles:
                            base_name = output_shm_name.split('_')[0]+"_"+output_shm_name.split('_')[1]
                            output = result.get_output(base_name)
                            
                            if output is not None:
                                output_data[base_name] = cudashm.get_contents_as_numpy(
                                    shm_handle, triton_to_np_dtype(output.datatype), shape
                                )
                            else:
                                print(f"{base_name} is missing in the response.")
                        
                        # Postprocess detections
                        detections = postprocess_output(
                            output_data["num_dets"], 
                            output_data["det_boxes"], 
                            output_data["det_scores"], 
                            output_data["det_classes"], 
                        ...
                        try:
                            ...
                            for (output_shm_name, shm_handle, _, _, _) in output_shm_handles:
